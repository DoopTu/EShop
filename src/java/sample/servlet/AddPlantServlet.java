...
public class AddPlantServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String pname = request.getParameter("addfullname");
            ...
            int status = 0;
            boolean flag = false;
            if ((("0".equals(addstatus)) || ("1".equals(addstatus)))) {
                status = Integer.parseInt(addstatus);
                flag = true;
            } else {
                request.setAttribute("msgstatus", "Status must be 0 or 1 !");
                flag = false;
            }
            int addCateID = Integer.parseInt(request.getParameter("addCateID")) ;
            if(flag == true){
                PlantDAO.addPlants(pname, addprice, addimg, addDescription, status, addCateID );
                request.setAttribute("msgadd", "Product is added!");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(AddPlantServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
